...
exports.NatSpecHoverProvider = void 0;
const vscode = require("vscode");
const tagDefinitions_1 = require("../utils/tagDefinitions");
class NatSpecHoverProvider {
    provideHover(document, position, token) {
        ...
        if (this.isNatSpecTag(line, word)) {
            const tagDef = tagDefinitions_1.NATSPEC_TAGS[word];
            if (tagDef) {
    ...
    createTagHover(tagDef, range) {
        ...
        markdown.appendCodeblock(`/// @${tagDef.name}: ${(0, tagDefinitions_1.getExampleValue)(tagDef)}`, 'solidity');
        return new vscode.Hover(markdown, range);
    }
}
exports.NatSpecHoverProvider = NatSpecHoverProvider;
